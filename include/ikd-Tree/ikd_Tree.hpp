...
public:
  ...
  void GetClosestPoint(PointType point, int k_nearest, PointVector &Nearest_Points, float max_dist = INFINITY);
...
template <typename PointType>
void KD_TREE<PointType>::GetClosestPoint(PointType point, int k_nearest, PointVector &Nearest_Points, float max_dist)
{
  MANUAL_HEAP q(2 * k_nearest);
  q.clear();
//  vector<float>().swap(Point_Distance);
