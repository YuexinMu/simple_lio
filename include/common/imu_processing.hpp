...
public:
  ...
  inline void Reset();
  inline void SetExtrinsic(const Vec3d &transl, const Mat3d &rot);
  inline void SetGyrCov(const Vec3d &scaler);
  inline void SetAccCov(const Vec3d &scaler);
  inline void SetGyrBiasCov(const Vec3d &b_g);
  inline void SetAccBiasCov(const Vec3d &b_a);
  inline void Process(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
...
private:
  inline void IMUInit(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N);
  inline void UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
...
void ImuProcess::SetExtrinsic(const Vec3d &transl, const Mat3d &rot) {
  Lidar_T_wrt_IMU_ = transl;
  Lidar_R_wrt_IMU_ = rot;
}

void ImuProcess::SetGyrCov(const Vec3d &scaler) { cov_gyr_scale_ = scaler; }

void ImuProcess::SetAccCov(const Vec3d &scaler) { cov_acc_scale_ = scaler; }

void ImuProcess::SetGyrBiasCov(const Vec3d &b_g) { cov_bias_gyr_ = b_g; }

void ImuProcess::SetAccBiasCov(const Vec3d &b_a) { cov_bias_acc_ = b_a; }
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
  ...
  const double &pcl_beg_time = meas.lidar_beg_time_;
  ...
  for (auto it_kp = IMUpose_.end() - 1; it_kp != IMUpose_.begin(); it_kp--) {
    auto head = it_kp - 1;
    auto tail = it_kp;
    R_imu = MatFromArray<double>(head->rot);
    vel_imu = VecFromArray<double>(head->vel);
    pos_imu = VecFromArray<double>(head->pos);
    acc_imu = VecFromArray<double>(tail->acc);
    angvel_avr = VecFromArray<double>(tail->gyr);
