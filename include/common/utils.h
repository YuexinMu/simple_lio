...
public:
  ...
  static void PrintAll() {
    ...
    for (const auto& r : records_) {
      double all_time = std::accumulate(r.second.time_usage_in_ms_.begin(), r.second.time_usage_in_ms_.end(), 0.0);
      LOG(INFO) << "> [ " << r.first << " ]"
                << " all time usage: " << all_time
                << " ms , average time usage: " << all_time / double(r.second.time_usage_in_ms_.size())
